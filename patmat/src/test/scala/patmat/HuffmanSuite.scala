...
class HuffmanSuite {
  ...
  @Test def `times function`:Unit =
    new TestTrees {
      assertEquals(List(('b', 3), ('a', 2), ('c', 1)), times(List('a', 'b', 'c', 'a', 'b', 'b')) )
    }

  @Test def `makeOrderedLeafList function`:Unit =
    assertEquals(List(Leaf('b', 2), Leaf('c', 3), Leaf('a', 4)), makeOrderedLeafList(List(('b', 2), ('a', 4), ('c', 3))))

  @Test def `singleton function`:Unit =
  ...
  @Test def `find out what the secret message says`: Unit = {
  ...
  @Test def `quickEncode produces same results as slower encode method`: Unit =
    new TestTrees {
